...
#
# Timing  : each image displays for 1795 s (~30 min) then a 5 s crossfade
#           bridges to the next → 30 min per wallpaper, N × 30 min for
#           one full loop. 20 assets → 10 h full loop.
#
...
# Write the XML manifest.
{
    ...
    if [ "$N" -eq 1 ]; then
        ...
        for i in "${!PNGS[@]}"; do
            ...
            next="${PNGS[$(( (i + 1) % N ))]}"
            printf '  <static>\n'
            printf '    <duration>1795.0</duration>\n'
            printf '    <file>%s</file>\n' "$current"
            printf '  </static>\n'
